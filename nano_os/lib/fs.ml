...
module Make (B : Block.BLOCK) : S with type block_t = B.t = struct
  ...
  (* On-disk layout:
     - Superblock at sector 0: magic(8) | count(4) | next_free(4)
     - Entry: name(56) | start(4) | size(4)
     - Data region starts after directory_sectors. *)
    ...
    | Ok (entries, next_free, _) ->
        ...
        if (not existing) && List.length entries >= max_files then Lwt.return (Error `No_space)
          ...
          if next_free_i + sectors_needed > disk_sectors then Lwt.return (Error `No_space)
            ...
            (* Overwrite is logical: we append new data at next_free and update
               the directory entry; old blocks are not reclaimed. *)
